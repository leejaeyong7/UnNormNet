...
class ImagenetHomo(HomographyDataset):
    ...
    def __init__(self, root, aug_config, extensions=IMG_EXTENSIONS, 
                is_valid_file=None, transform=None, target_transform=None,
                two_crop=True):

        with open(aug_config, 'r') as f:
            try:
                config = yaml.safe_load(f)
            except yaml.YAMLError as exc:
                logging.error("Invalid YAML")
                logging.error(exc)
        super(ImagenetHomo, self).__init__(config)
    ...
    def __getitem__(self, index):
        ...
        i1 = Image.fromarray(augmented[0])
        i2 = Image.fromarray(augmented[1])

        # TODO: transform should be color only!
        if self.transform is not None:
            img = self.transform(i1)
